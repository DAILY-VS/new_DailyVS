...
class PollDetailView(APIView):
    def get(self, request, poll_id):
        ...
        if user.is_authenticated and poll.user_id.filter(id=user).exists(): #user에 post_id를 넣지말고 post에 user_id를 찾음
            ...
            loop_count = poll.choice_set.count() #이거 빼고 2로 넣어도 됨
...
#     user = request.user
#     if poll.poll_like.filter(pk=user.pk).exists():
#         poll.poll_like.remove(user)
#     else:
#         poll.poll_like.add(user)

        if poll.poll_like.filter(id=user.id).exists(): # 좋아요 모델 사용
            poll.poll_like.remove(user)
            message = "좋아요 취소"
...
# PollLike = PollLike.objects.get(id=poll.id)
# pollLike.add(user)


#     if request.user.is_authenticated:
#         if poll.user_id.filter(id=user.id).exists():
#             user_likes_poll = True

#     if request.user.is_authenticated:
#         if comment.user_info.filter(id=user.id).exists():
#             user_likes_poll = True
...
def comment_write_view(request, poll_id):
    ...
    parent_comment_id = request.POST.get("parent_comment_id") #대댓글
    ...
    if content:
        ...
        #poll.comments_count() = comments.count()
        poll.save()
